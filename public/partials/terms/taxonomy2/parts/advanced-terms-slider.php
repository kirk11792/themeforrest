					...
					if($instance->params['enable_content_wrapper']){
						...
						if($instance->params['layout'] == 4){
							if(isset($instance->params['count']) && $instance->params['count']){
								if(isset($instance->params['count_with_text']) && $instance->params['count_with_text']){
									...
									echo '<div class="advanced-term-slider-item-numbers" style="position: '.$instance->params['counter_position'].'">'.$term_count.'&nbsp;'. $count_text .'</div>';
								}else{
									echo '<div class="advanced-term-slider-item-numbers" style="position: '.$instance->params['counter_position'].'">'. $term_count .'</div>';
								}
							}
						}
						if($instance->params['layout'] == 1 || $instance->params['layout'] == 4){
							if(isset($instance->params['icon_type']) && $instance->params['icon_type'] == 'font_svg'){
								echo '<div class="advanced-term-slider-item-icon" style="position: '.$instance->params['icon_position'].'">';
								...
								if(isset($instance->params['icon_image']) && $instance->params['icon_image']['url']){
									echo '<div class="advanced-term-slider-item-icon" style="position: '.$instance->params['icon_position'].'"><img src="'. $instance->params['icon_image']['url'] .'" alt="'.$term->name.'"/></div>';
								}
							}
						}
						...
						echo '<div class="advanced-term-slider-item-title"  style="position: '.$instance->params['title_position'].'">';
